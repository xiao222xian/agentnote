...
**CoPaw** (Co Personal Agent Workstation) 不仅仅是一个聊天机器人，它是你养在本地或私有云上的 **AI 助手工作台**。

如果说传统的 AI 助手是租来的“办公室”，那么 CoPaw 就是你**亲手打造的“数字庄园”**。它深度集成了模型管理、多渠道接入、技能扩展和长期记忆系统，旨在为你提供一个私密、强大且具备自主执行能力的 AI 伙伴。

### 🎯 核心场景

* **全平台沟通**：在飞书、QQ、Discord 或 Slack 中直接唤醒你的专属 AI。
* **深度个性化**：让 AI 长期记住你的职业偏好、写作风格和生活习惯。
* **自动化秘书**：设定定时任务，让它每天准时为你推送行业简报或任务提醒。
* **数据主权**：支持完全本地化部署，敏感数据不经过任何第三方平台，确保绝对隐私。

---

## ⚖️ CoPaw vs. OpenClaw：该选哪一个？

两者都是优秀的自托管 Agent 平台，但它们的“性格”截然不同：

| 特性 | **CoPaw (个人工作站)** | **OpenClaw (任务执行中枢)** |
| --- | --- | --- |
| **核心定位** | 偏向“陪伴与协作”，强调工作台体验 | 偏向“开发者与执行”，强调工具链 |
| **控制方式** | 优雅的 **Web 控制台 (Port 8088)** | 强大的 **CLI 命令行与网关** |
| **技术栈** | 基于 Python 生态 (AgentScope) | 偏向 Node.js/跨语言插件体系 |
| **典型功能** | 定时心跳、摘要生成、Skills 自动加载 | 任务执行链、复杂的插件化扩展 |
| **最佳受众** | 追求日常易用性、网页控制、长期记忆的用户 | 追求自动化流程、开发者控制感的极客 |

> **一句话建议**：如果你想要一个像“数字管家”一样的网页版助手，选 **CoPaw**；如果你想要一个像“瑞士军刀”一样的命令行工具，选 **OpenClaw**。

---

## 🏗️ 核心架构与功能

1. **多渠道接入 (Multi-Channel)**：一键桥接飞书、QQ、Discord 等，打破应用孤岛。
2. **智能大脑 (Brain)**：支持云端主流模型及基于 `llama.cpp` 或 `MLX` 的本地推理模型。
3. **技能扩展 (Skills)**：类似插件系统，支持快速加载自定义工具。
4. **定时任务 (Cron Jobs)**：内置心跳检查与定时摘要，让 Agent 具备时间观念。
5. **灵活部署**：支持本地、Docker、阿里云 ECS 等多种环境。
...
启动后，访问：`http://127.0.0.1:8088/` 即可进入 Web 控制台。
...
*配置、记忆和 Skills 均会持久化保存在 `copaw-data` 中。*

### 方式 C：一键安装脚本

官方提供 `one-line install`，适合零基础用户。请访问 [官方快速开始页面](https://www.google.com/search?q=https://docs.agentscope.io/copaw) 获取最新的脚本命令。
...
在 `copaw init` 过程中，根据提示选择模型供应商（如 OpenAI, Anthropic, DashScope），并输入你的 **API Key**。

### 本地模型 (隐私至上)

CoPaw 对本地模型极其友好，支持 `llama.cpp` 或 `MLX` 后端：

* **无需 API Key**：直接加载 GGUF 格式模型。
* **本地下载示例**：
```bash
# 示例：通过内置命令下载 Qwen 模型
...
* **Skills 路径**：自定义技能可放入 `/app/working/skills` 文件夹，重启后自动加载。
* **定时任务**：在控制台中可直接编辑任务流，设定 Agent 的唤醒频率。
